...
def player_attack(player: dict, enemy: dict) -> None:
...
def use_potion(player: dict) -> None:
  if player["potions"] > 0:
    ...
    if player["life"] > player["max_life"]:
      ...
      print(
          f"The player {player['name']} has exceeded the maximum value of life,"
          f" life was set to: {player['life']}"
      )
    else:
...
if player["life"] > 0:
  print(
      f"*******Congratulations {player['name']}, you have defeated ALL the"
      " enemies!*******"
  )